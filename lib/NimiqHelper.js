...
class NimiqHelper extends Observable {
    async connect() {
        ...
        let port = parseInt(NIMIQ_PORT, 0)
        let networkConfig
        switch (NIMIQ_PROTOCOL) {
        case 'wss':
            networkConfig = new Nimiq.WssNetworkConfig(DOMAIN_NAME, port, SSL_KEY, SSL_CERT, false)
            break
        case 'ws':
            networkConfig = new Nimiq.WsNetworkConfig(DOMAIN_NAME, port, config.reverseProxy)
            break
        case 'dumb':
        ...
        let consensusFunc
        switch (NIMIQ_CONSENSUS) {
        case 'full':
            consensusFunc = Nimiq.Consensus.full
            break
        case 'light':
            consensusFunc = Nimiq.Consensus.light
            break
        default:
            throw new Error(`Unsupported consensus type: ${NIMIQ_CONSENSUS}`)
        }

        Log.i('Connecting to Nimiq Network: ' + NIMIQ_NETWORK + ' Protocol:' + NIMIQ_PROTOCOL)
        this.$.consensus = await consensusFunc(networkConfig)
    ...
    _transactionToObj(tx, block, i) {
        let buf = Nimiq.BufferUtils.toAscii(tx.data)
        return {
            hash: tx.hash().toHex(),
            ...
            // confirmations: tx.validityStartHeight,
            transactionIndex: i,
